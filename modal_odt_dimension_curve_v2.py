...
app = modal.App("xvla-odt-dimension-curve-v2")
...
controller_image = image.add_local_file("tests/fixtures/modal_odt_controller_zero_component_capture.json", "/root/odt/tests/fixtures/modal_odt_controller_zero_component_capture.json").add_local_file("tests/fixtures/modal_odt_controller_real_gate_capture.json", "/root/odt/tests/fixtures/modal_odt_controller_real_gate_capture.json").add_local_file("modal_odt_dimension_curve_controller.py", "/root/odt/modal_odt_dimension_curve_controller.py").add_local_file("tests/test_modal_odt_dimension_curve_controller.py", "/root/odt/tests/test_modal_odt_dimension_curve_controller.py")
...
SOURCE_BUNDLE_DIRECTORY = "tmp/odt_modal_curve_v1/source_v6"
SOURCE_BUNDLE_SHA256 = "9005fe5cd4a93cd0b459093b42ff1cf20a1b29ac8a02d39d7fa76921f7b921c1"
worker_image = image.add_local_dir(SOURCE_BUNDLE_DIRECTORY, "/root/odt")
worker_test_image = worker_image.add_local_file("tests/test_modal_odt_dimension_curve_oracle.py", "/root/odt/tests/test_modal_odt_dimension_curve_oracle.py").add_local_file("tests/test_modal_odt_dimension_curve_worker.py", "/root/odt/tests/test_modal_odt_dimension_curve_worker.py").add_local_file("tests/test_modal_odt_dimension_curve_reduced.py", "/root/odt/tests/test_modal_odt_dimension_curve_reduced.py")


def _execute_policy(removal: int, run_name: str, artifact_manifest_sha256: str = "", receipt_sha256: str = "", smoke: bool = False, source_dtype: str = "float64") -> dict:
    import contextlib
    import dataclasses
    import hashlib
    import importlib.metadata
    ...
    from modal_odt_dimension_curve_controller import install_controller, install_prohibited_route_guards, COUNTS
    for key in COUNTS:
        COUNTS[key] = 0.0 if key.startswith("largest_") else 0
    ...
    if source_dtype not in ("float32", "float64") or (removal != 0 and source_dtype != "float64"):
        raise ValueError("worker source/execution precision differs")
    ...
    identity = {"schema": "xvla_modal_odt_fixed20_closed_loop_v1", "requested_removal_percent": removal,
        ...
        "execution_dtype": source_dtype, "primary_curve_comparator": source_dtype == "float64",
        "execution_backend": "source_policy" if removal == 0 else "compiled_direct_tensor_contractions",
    ...
    try:
        with contextlib.ExitStack() as stack:
            ...
            mapped = None
            if removal == 0:
                model = load_source(inputs, protocol["configuration"], source_dtype=source_dtype)
            else:
                ...
                from modal_odt_dimension_curve_oracle import RealPanelCheckedCompiledExecutor
                mapped = stack.enter_context(RealPanelCheckedCompiledExecutor(mapped,
                    training["normalization"]["action_mean"], training["normalization"]["action_std"], publish, artifact_manifest_sha256))
            ...
            if removal == 0:
                ...
                if not mapped.verified:
                    raise RuntimeError("reduced policy did not pass its real-panel arithmetic oracle")
                identity["verified_real_panel_oracle"] = True
...
@app.function(image=worker_image, gpu="L4", cpu=(16.0, 16.0), memory=(32768, 32768), timeout=7200, max_containers=2, volumes={"/vol": volume})
def baseline(run_name: str, smoke: bool = False, source_dtype: str = "float64") -> dict:
    return _execute_policy(0, run_name, smoke=smoke, source_dtype=source_dtype)
...
def worker_tests() -> dict:
    import subprocess
    result = subprocess.run(["python", "-m", "pytest", "-q", "/root/odt/tests/test_modal_odt_dimension_curve_worker.py", "/root/odt/tests/test_modal_odt_dimension_curve_reduced.py", "/root/odt/tests/test_modal_odt_dimension_curve_oracle.py"], capture_output=True, text=True, timeout=240)
    if result.returncode:
        raise RuntimeError(result.stdout + result.stderr)
    return {"passed": True, "stdout": result.stdout, "stderr": result.stderr}
...
def baseline_smoke() -> None:
    print(baseline.remote("baseline_smoke_v4", smoke=True, source_dtype="float64"))
...
def evaluate(
    ...
    if removal == 0:
        ...
        result = baseline.remote(run_name, smoke=smoke, source_dtype=source_dtype)
    else:

"""Float64 dense localFFN inside the unchanged native float32 policy."""
import argparse
import json
        ...
        with torch.inference_mode():
            for start in range(0,count,batch_size):
                ...
                try:
                    ...
                    status[start:stop]=2 if "denominator" in str(error) else 3
                    failure={"first_uncompleted_image_index":start,"message":str(error),"remaining_frames_not_attempted":count-start}
                    break
    finally:
...
def action_metrics(result, baseline):
    ...
    attempted_invalid=attempted & ~result["valid"]
    complete=bool(np.all(attempted))
    metrics={"frames":len(valid),"attempted_invalid_frames":int(np.sum(attempted_invalid)),
        ...
        "invalid_probability_if_fully_attempted":float(np.mean(~result["valid"])) if complete else None,
...
def execute(args):
    ...
    write_json(args.output/"manifest.json",{"workflow_completed":True,
        ...
        "schema":"odt-ffn-native-actions-v1","outcomes":outcomes,
        ...
        "status_codes":{"not_attempted":0,"valid":1,"denominator_invalid":2,"execution_failure":3},

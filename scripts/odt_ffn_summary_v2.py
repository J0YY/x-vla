...
def audit_analysis():
    ...
    files = (Path(__file__), Path(__file__).with_name("test_odt_ffn_summary_v2.py"),
             ROOT / "research/odt_reference/run_tests.py")
    imports = {"argparse", "ast", "hashlib", "json", "math", "numpy", "unittest"}
    modules = {"pathlib", "research.odt_reference.run_tests", "scripts", "scripts.odt_ffn_summary_v2", "unittest.mock"}
    hashes = {}
    for path in files:
...
def array_hash(value):
    # The native panel hashes the contiguous representation, including its
    # promotion of scalar arrays to a one-element shape.
    value = np.ascontiguousarray(value)
    if value.dtype.hasobject:
        raise ValueError("object arrays are outside the authenticated boundary")
...
def summarize(actions, panel_path, metadata_path, variants, output):
    ...
    report = {"schema": "odt-ffn-summary-v2", "complete": True, "arms": summaries, "contrasts": contrasts,

...
On all ten LIBERO-Object tasks, the $20.1$M-parameter policy reaches $93.7\%\pm0.8\%$ with a
convolutional encoder under the published three-seed protocol. Averaging three fixed
checkpoints reaches $96.2\%$ at three times the inference cost.
...
median fidelity ratio $2.44$. At the policy's visual bond, retaining $96$ of $384$ causally
...
random projector across action groups and checkpoints. The evidence establishes competitive
specialist capability, exact layerwise access, and a compact causal policy bottleneck. It does
not establish broad robotic generalization.
\end{abstract}
...
exact paired $p\leq 2.4\times10^{-4}$. Offline fidelity uses $1{,}024$ discovery samples and
$1{,}024$ disjoint evaluation samples per checkpoint. Against the median random projector, the
...
weight-derived discovery and a replicated causal bottleneck in a competitive continuous
...
The scope is deliberately specific. LIBERO-Object is one specialist suite, so these results do
not establish broad robotic generalization. Exact reconstruction is certified layer by layer,
while the full-policy visual ranking also uses cached activations and action gradients. The
three-checkpoint ensemble trades three forward passes for its strongest capability result.

Within that scope, polynomial and rational restrictions are compatible with competitive
closed-loop control and provide a precise weight-derived analysis surface. The same trained
policies support the capability, exact reconstruction, and causal-bottleneck results, linking
the algebraic model artifact to behavior rather than treating decomposability as an architectural
promise alone.

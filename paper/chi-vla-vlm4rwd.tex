...
\begin{abstract}
Can a capable robot policy be built entirely from learned operations that admit direct tensor
decomposition? We introduce \chivla, a compact vision-language-action policy whose learned map
uses bilinear attention, bilinear feed-forward blocks, linear projections, and rational
per-instance normalization. The complete deployed map is therefore a rational tensor program.
A mechanical audit of a trained checkpoint finds no softmax, standard activation, or ordinary
normalization operation, and explicit reconstructions match deployed computation near the
float32 precision floor.
On all ten LIBERO-Object tasks, a $20.1$M-parameter policy trained from scratch reaches
$92.8\%\pm2.1\%$ with a ViT encoder and $93.7\%\pm0.8\%$ with a convolutional encoder under
the published $280$-step, $50$-trial, three-seed protocol. Averaging three convolutional
checkpoints reaches $96.2\%$ at three times the inference cost. These results establish
competitive specialist capability, not broad robotic generalization.
We then ask what the algebraic restriction provides. We derive an exact weight-based
decomposition of softmax-free bilinear attention, including the rational query-key
normalization used by the policy. In a fixed-control screen of $36$ trained block-head pairs,
all $36$ favor the weight-derived rank-$128$ subspace over equal-rank random subspaces. The
median fidelity ratio is $2.44$, the mean is $3.03$, and the maximum is $10.14$.
At the policy's visual bond, retaining $64$ of $384$ causally ranked dimensions preserves
$18/20$ closed-loop successes, compared with $19/20$ for the full representation and $0/20$
for a random subspace. An expanded $80$-trial counterfactual instruction test nevertheless
exposes a scene-identity shortcut. The present evidence establishes capability and exact
layerwise structure. It does not yet establish negligible cost against a matched conventional
twin, semantic diagnosis from weights alone, or selective coefficient-level repair.
\end{abstract}
...
\midrule
Near-zero capability cost &
Two fully rational $20.1$M policies reach $92.8\%$ and $93.7\%$ mean success. &
Same-skeleton conventional and partial-conversion twins under paired budgets. \\
...
structure are established. The matched cost and coefficient-surgery claims remain open.}
\label{tab:thesis}
\end{table}
...
\paragraph{1. Controlled conversion cost.}
Train a same-skeleton conventional model with softmax attention, a standard gated FFN, and
RMSNorm. Add partial-conversion controls. Use paired seeds and predeclare a three-point
noninferiority margin. Match observations, token layout, action decoder, demonstrations,
augmentation, optimizer, updates, evaluation states, and hyperparameter-search budget. Report
parameters, FLOPs, throughput, memory, and latency. This is the missing experiment for any
claim that convertibility is nearly free.
...
Five limits determine the next version of the paper. First, no same-skeleton conventional twin
has yet measured the exact capability and compute cost of conversion. Second, the benchmark is
one narrow suite and the policy fails a counterfactual instruction test. Third, exact
...
prevent a compact policy from reaching competitive LIBERO-Object capability, and they provide
a precise weight-derived analysis surface. Establishing negligible conversion cost, broad

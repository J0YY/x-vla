...
checkpoints, retaining $96$ of $384$ causally ranked visual dimensions succeeds in $50/60$
...
reconstruction error than the median random projector across action groups and checkpoints.
The result is a capable and structurally inspectable specialist with a compact causal policy
bottleneck.
\end{abstract}
...
We test two concrete requirements on the same trained policies: competitive closed-loop
capability and reproducible structure that has a causal behavioral consequence.
...
\begin{enumerate}
  ...
  \item We show a causal closed-loop consequence. A selected $96$-dimensional visual subspace
...
whether the algebraic policy makes weight-derived structure direct, stable, and behaviorally
testable.
...
We next test whether the identified structure affects closed-loop behavior.
...
Appendix Figure~\ref{fig:causal} reports the action-balanced rank screen. Full representations
succeed in $17/20$, $15/20$, and $20/20$ trials across checkpoints. Selected rank-$96$
...
random-control comparison has the same selected-over-random direction, with exact paired
$p\leq 2.4\times10^{-4}$. Offline fidelity uses $1{,}024$ discovery samples and $1{,}024$
disjoint evaluation samples per checkpoint. The selected basis has $53.0$--$110.6\times$
lower translation error, $37.5$--$62.1\times$ lower rotation error, and
$33.5$--$103.0\times$ lower gripper error than the median equal-rank random projector. This
...
cached activations and action gradients. Within that scope, rational restrictions support
competitive LIBERO-Object control, exact trained-weight certificates, and a rank-$96$ causal
bottleneck that retains $49$ of $52$ full-policy successes across three checkpoints.
...
\caption{\textbf{One quarter of the visual bond preserves policy behavior.} Each checkpoint
uses four tasks and five paired canonical trials per task. Rank $96$ retains $49$ of $52$

...
subspaces beat equal-rank random controls in all $36$ primary block-head tests. Retaining $64$
of $384$ causally ranked visual dimensions preserves $18/20$ successes, while a random subspace
preserves none. A $300$-pair instruction intervention reliably changes motion but rarely
...
seed-0 task gap is therefore checkpoint-specific. On one A30, eager latency is $38.05$ versus
$7.14$ ms. Compilation
...
Conventional seeds 1 and 2 completed at only $9.0\%$ and $8.2\%$, despite low offline losses.
This exposes severe optimization variance rather than a stable across-seed conversion cost.
Matched \chivla\ evaluations and partial-conversion controls remain necessary.
...
$41.8\times$ for the gripper. Appendix Figure~\ref{fig:causal} reports the full comparison.
In closed loop, the full representation succeeds in $19/20$ trials, the selected subspace in
$18/20$, and a random subspace in $0/20$.
...
subspace test uses one checkpoint and twenty trials, while a fifteen-configuration discovery
sweep yields no coefficient edit that passes both selectivity gates. The defensible result is therefore

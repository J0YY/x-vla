...
{\large Protocol-aligned control, exact weight analysis, and causal representation tests}}
\author{}
\date{}
...
$93.7\%\pm0.8\%$ over three seeds under the published protocol. In a separate ViT
...
checkpoints, an exploratory rank screen finds that retaining $96$ of $384$ causally ranked
visual dimensions succeeds in $50/60$
matched trials, compared with $52/60$ for the full representation. Three equal-rank random
controls reach only $1/60$, $3/60$, and $3/60$. The selected subspace retains $49$ of the $52$
full-policy successes. On disjoint cache samples, it has $33.5$--$110.6\times$ lower
reconstruction error than the median across three random projectors, computed separately by
action group. These exact and causal analyses are complementary. They do not yet map the exact
attention factors directly to the causal visual basis. The result is a capable and structurally
inspectable specialist family with a compact exploratory policy bottleneck.
\end{abstract}
...
operation is polynomial or rational, while supporting exact weight analysis and causal
...
the exact attention audit and causal representation screen.
...
\begin{enumerate}
  ...
  \item In a separate exploratory rank screen, a selected $96$-dimensional visual subspace
        retains $49$ of $52$ full-policy successes across three checkpoints. Three equal-rank
        random controls reach only $1/60$, $3/60$, and $3/60$.
\end{enumerate}
...
whether the algebraic policy makes exact weight structure directly accessible and supports
separate causal tests of its internal representations.
...
% ============================================================================
\section{Causal policy structure}
\label{sec:causal}
% ============================================================================
...
At the visual bond before the joint backbone, we build one shared action-balanced ranking. We
form one Jacobian Gram per action coordinate, trace-normalize the seven Grams, and average them.
A subspace is a smaller coordinate system inside the model's
...
The exploratory screen tests ranks $64$, $96$, $128$, and $192$ on tasks 4 to 7. Appendix
Figure~\ref{fig:causal-rank-curve} reports the full curve. Rank $96$ is the smallest tested width
with more than $90\%$ conditional retention while random success remains below $5\%$. Tasks 8
and 9 are reserved for a frozen confirmation and are not used here. At rank $96$, full
representations succeed in $17/20$, $15/20$, and $20/20$ trials across checkpoints. Selected rank-$96$
subspaces reach $17/20$, $14/20$, and $19/20$, and retain $49$ of the $52$ exact full-policy
successes. Their conditional retention is $94.1\%$, $93.3\%$, and $95.0\%$. Three random
controls per checkpoint total only $1/60$, $3/60$, and $3/60$. Every checkpoint and
random-control comparison has the same selected-over-random direction. Within each fixed
checkpoint, the descriptive exact paired tests have $p<2.5\times10^{-4}$. Offline fidelity uses
$1{,}024$ discovery samples and $1{,}024$ disjoint evaluation samples per checkpoint. The
selected basis has $53.0$--$110.6\times$
lower translation error, $37.5$--$62.1\times$ lower rotation error, and
$33.5$--$103.0\times$ lower gripper error than the median across three random projectors,
computed separately by action group. This
repairs sample reuse but remains an in-cache diagnostic. This data-driven full-policy ranking
complements the exact layerwise analysis in Section~\ref{sec:exactattention}.
...
Exact reconstruction is certified layer by layer, while the full-policy visual ranking also uses
cached activations and action gradients. The capability result uses convolutional checkpoints,
while the exact and causal analyses use separate ViT checkpoints. The latter are complementary,
not a demonstrated direct mapping. Evaluation uses fixed task instructions and does not
establish robust instruction grounding. Within that scope, rational restrictions support
strong LIBERO-Object control, exact trained-weight certificates, and an exploratory rank-$96$
causal bottleneck that retains $49$ of $52$ full-policy successes across three checkpoints. We
do not claim a direct coefficient-edit interface.
...
\subsection{Rank screen and selectivity}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../paper_figures/appendix_causal_rank_curve.pdf}
\caption{\textbf{Rank $96$ is the empirical selectivity knee.} Selected subspaces use $60$
closed-loop trials per rank across three checkpoints. Random results pool three equal-rank
controls per checkpoint, for $180$ trials per rank. Each rank has its own paired full-policy
condition. Rank $96$ is the smallest width above $90\%$ conditional retention, while random
success remains below $5\%$.}
\label{fig:causal-rank-curve}
\end{figure}

The selected subspace rises from $30/60$ successes at rank $64$ to $50/60$ at rank $96$.
Wider ranks add little selected success, reaching $50/60$ and $51/60$ at ranks $128$ and
$192$. In contrast, random success rises from $7/180$ at rank $96$ to $50/180$ and $99/180$.
Conditional retention is $54.9\%$, $94.2\%$, $96.0\%$, and $96.2\%$ across the four ranks.
Thus rank $96$ preserves nearly all baseline successes before equal-width random subspaces
become broadly capable.
...
\caption{\textbf{One quarter of the visual bond preserves policy behavior in the exploratory
rank screen.} Each checkpoint uses four tasks and five paired canonical trials per task. Rank
$96$ retains $49$ of $52$
full-policy successes. Three equal-rank random controls reach $1/60$, $3/60$, and $3/60$.
The pooled interval is descriptive because trials within a checkpoint share learned weights.}
\label{fig:causal}
\end{figure}

#!/usr/bin/env python3
"""Create the outcome-independent instruction-embedding-ablation manifest."""
...
def main() -> None:
    ...
    for task_index in range(10):
        ...
        if len(init_states) < 40:
            raise RuntimeError(f"Task {task_index} has fewer than 40 canonical states")
        ...
        try:
            for episode in range(30, 40):
                ...
                bodies = sorted(scene_object_bodies(observation))
                if len(bodies) != 6:
                ...
                mappings.append(
                    {
                        ...
                        "instruction_token_count": PROTOCOL["instruction_token_count"],
                        ...
                        "settled_model_image_sha256": hash_array(
                            np.asarray(image, dtype=np.uint8)
                        ),
                        "settled_model_state_sha256": hash_array(
    ...
    observed_keys = {
        (int(row["task_index"]), int(row["episode"])) for row in mappings
    }
    ...
    output = {
        ...
        "checkpoints": {str(seed): row for seed, row in CHECKPOINTS.items()},

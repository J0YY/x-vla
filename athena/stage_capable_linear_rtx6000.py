#!/usr/bin/env python3
"""Create the RTX 6000 stage only from its immutable verified prestage."""
...
EXPECTED_PRESTAGE_ROOT = Path("/work/joy/x-vla-capable-linear-rtx6000-prestage-v3")
STAGE_ROOT = Path("/work/joy/x-vla-capable-linear-rtx6000-v3")
PRESTAGE_MANIFEST = SOURCE_ROOT / "athena/capable_linear_rtx6000_prestage_manifest.json"
...
EXPECTED_FROZEN_B1C_STAGE_LEDGER_SHA256 = (
    "e4716a413bc742943b623ddebd39783bb54c40558b59446c99998aa07892b8c3"
)
EXPECTED_FROZEN_B1C_CAPABILITY_MANIFEST_SHA256 = (
...
def _verified_prestage() -> dict[str, Any]:
    ...
    shared_sources = manifest.get("shared_source_sha256")
    if (
        manifest.get("schema") != PRESTAGE_SCHEMA
        or not isinstance(sections, dict)
        or not isinstance(closure, dict)
        or not isinstance(source_origin, dict)
        or not isinstance(shared_sources, dict)
    ):
        raise RuntimeError("prestage manifest schema or maps differ")
    expected_sections = {
        "cross_stage_source_sha256",
    ...
    if (
        set(source_origin) != set(closure)
        or not all(
            value in {"frozen_input", "new_workshop_authority", "frozen_b1c_v2"}
            for value in source_origin.values()
        )
        or manifest.get("frozen_b1c_stage_ledger_sha256")
        != EXPECTED_FROZEN_B1C_STAGE_LEDGER_SHA256
        or manifest.get("frozen_b1c_capability_manifest_sha256")
        != EXPECTED_FROZEN_B1C_CAPABILITY_MANIFEST_SHA256
        or len(shared_sources) != 19
        or any(closure.get(relative) != digest for relative, digest in shared_sources.items())
        or any(source_origin.get(relative) != "frozen_b1c_v2" for relative in shared_sources)
    ):
        raise RuntimeError("prestage mixed-origin provenance differs")
    input_members = set(sections["input_sha256"])
    if any(source_origin.get(relative) != "frozen_input" for relative in input_members):
        raise RuntimeError("prestage input origin inventory differs")
    non_input_non_shared = set(closure) - input_members - set(shared_sources)
    if any(
        source_origin.get(relative) != "new_workshop_authority"
        for relative in non_input_non_shared
    ):
        raise RuntimeError("prestage new-authority origin inventory differs")
    if manifest.get("bundle_sha256") != _canonical_sha256(dict(sorted(closure.items()))):
        raise RuntimeError("prestage bundle differs")
    required = {
        "athena/stage_capable_linear_rtx6000.py",
        "scripts/__init__.py",
...
def _audit_sections() -> dict[str, Any]:
    ...
    cross_stage_entrypoints = (
        SOURCE_ROOT / "scripts/attest_capable_linear_rtx6000_cross_stage.py",
        SOURCE_ROOT / "scripts/run_capable_linear_rtx6000_capability.py",
        SOURCE_ROOT / "scripts/run_capable_linear_rtx6000_tests.py",
        SOURCE_ROOT / "tests/test_capable_linear_rtx6000_cross_stage.py",
        SOURCE_ROOT / "tests/test_capable_linear_rtx6000_capability.py",
    )
    if len(cross_stage_entrypoints) != len({path.resolve() for path in cross_stage_entrypoints}):
        raise RuntimeError("cross-stage staging entrypoint inventory contains duplicates")
    capability_entrypoints = (
        SOURCE_ROOT / "scripts/run_capable_linear_rtx6000_capability.py",
        SOURCE_ROOT / "tests/test_capable_linear_rtx6000_capability.py",
    )
    cross_stage = audit_direct_only_launch(
        SOURCE_ROOT, cross_stage_entrypoints, require_direct_qr=False
    )
    capability = audit_direct_only_launch(
        SOURCE_ROOT, capability_entrypoints, require_direct_qr=False
    )
    if cross_stage["source_sha256"] != sections["cross_stage_source_sha256"]:
        raise RuntimeError("prestage cross-stage section differs from transitive audit")
    ...
    for label, audit in (("cross-stage", cross_stage), ("capability", capability)):
        for field in (
    ...
    return {"cross_stage": cross_stage, "capability": capability}
...
def main() -> None:
    ...
    closure["inputs/rtx6000_prestage_manifest.json"] = _sha256(PRESTAGE_MANIFEST)
    closure["inputs/rtx6000_prestage_ledger.json"] = _sha256(PRESTAGE_LEDGER)
    STAGE_ROOT.mkdir(mode=0o755)
    for relative, digest in sorted(closure.items()):
        source = (
            PRESTAGE_MANIFEST
            if relative == "inputs/rtx6000_prestage_manifest.json"
            else PRESTAGE_LEDGER
            if relative == "inputs/rtx6000_prestage_ledger.json"
            else _safe_member(SOURCE_ROOT, relative)
        )
        _copy(source, STAGE_ROOT / relative, digest)

    results_root = STAGE_ROOT / "athena/results"
    results = results_root / "capable_linear_rtx6000"
    ...
    ledger_path = STAGE_ROOT / "athena/capable_linear_rtx6000_stage.sha256"
    ...
    payload = {
        "schema": "xvla_capable_linear_rtx6000_stage_v3",
        ...
        "cross_stage_static_audit": audits["cross_stage"],
        "capability_static_audit": audits["capability"],
        "no_job_submitted": True,
        "next_command": f"bash {STAGE_ROOT}/athena/submit_capable_linear_rtx6000.sh",
    }

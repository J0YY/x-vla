...
ffn_summary=results/conv_joint_ffn_certificate_v1_summary.json
...
job_state() {
  sacct -j "$1" --starttime 2026-08-20 --noheader --parsable2 \
    --allocations --format=State | head -1 | cut -d'|' -f1
}

provenance_state=$(job_state "$provenance_job")
if [[ "$provenance_state" != "COMPLETED" || ! -s "$provenance_result" ]]; then
  echo "Refusing to launch without completed Object provenance job $provenance_job" >&2
  exit 4
fi

ffn_summary_state=$(job_state "$ffn_summary_job")
case "$ffn_summary_state" in
  FAILED*|CANCELLED*|TIMEOUT*|OUT_OF_MEMORY*|NODE_FAIL*|DEPENDENCY*)
    echo "Refusing to launch: FFN summary job $ffn_summary_job is $ffn_summary_state" >&2
    exit 4
    ;;
  COMPLETED)
    if [[ ! -s "$ffn_summary" ]]; then
      echo "Refusing to launch: completed FFN summary has no result" >&2
      exit 4
    fi
    ;;
  RUNNING|PENDING|CONFIGURING|COMPLETING)
    ;;
  *)
    echo "Refusing to launch: unexpected FFN summary state '$ffn_summary_state'" >&2
    exit 4
    ;;
esac
...
for output in \
  results/conv_joint_block_certificate_v1_smoke_ddp2_a6000.json \
  results/conv_joint_block_certificate_v1_s0.json \
  results/conv_joint_block_certificate_v1_s1.json \
  results/conv_joint_block_certificate_v1_s2.json \
  results/conv_joint_block_certificate_v1_summary.json; do
  if [[ -e "$output" ]]; then
...
smoke_job=$("${gpu_submit[@]}" \
  --job-name=xvla-conv-joint-block-v1-smoke-a6000 \
  ...
  --output results/conv_joint_block_certificate_v1_smoke_ddp2_a6000.json \
  --mode strict_smoke)
...
for seed in 0 1 2; do
  full_jobs[$seed]=$("${gpu_submit[@]}" \
    ...
    --job-name="xvla-conv-joint-block-v1-s${seed}-a6000" \
    athena/slurm_conv_joint_block_certificate.sbatch \
    ...
    --attention-summary "$attention_summary" \
    --rational-norm-summary "$rational_norm_summary" \
    --output "results/conv_joint_block_certificate_v1_s${seed}.json" \
    --mode full)
done

dependency="afterok:${full_jobs[0]}:${full_jobs[1]}:${full_jobs[2]}:${ffn_summary_job}"
summary_job=$(sbatch --parsable \
  --dependency="$dependency" \
  --job-name=xvla-conv-joint-block-v1-summary \
  athena/slurm_summarize_conv_joint_block_certificate.sbatch \
  --result results/conv_joint_block_certificate_v1_s0.json \
  --result results/conv_joint_block_certificate_v1_s1.json \
  --result results/conv_joint_block_certificate_v1_s2.json \
  --checkpoint "${checkpoints[0]}" \
  ...
  --attention-summary "$attention_summary" \
  --rational-norm-summary "$rational_norm_summary" \
  --ffn-summary "$ffn_summary" \
  --output results/conv_joint_block_certificate_v1_summary.json)

echo "provenance $provenance_job $provenance_state"
echo "ffn_summary $ffn_summary_job $ffn_summary_state"

#!/usr/bin/env python3
"""Run the frozen closed-loop instruction-embedding-path ablation."""
...
def strict_pre_cuda_validation(args: argparse.Namespace, root: Path) -> dict[str, Any]:
    ...
    if args.mode == "strict_smoke":
        ...
        if observed != expected:
            raise ValueError("Strict smoke must cover episode 30 of all tasks at seed 0")
        ...
        if (args.episode_start, args.eps_per_task, args.expected_rollouts) != (30, 10, 40):
            raise ValueError("Full shard must contain 20 paired states and 40 rollouts")
        expected_output = root / (
            f"results/instruction_embedding_ablation_v1_s{args.checkpoint_seed}_"
    ...
    expected_common = os.environ.get("XVLA_INSTRUCTION_EMBEDDING_COMMON_SHA256")
    expected_runner = os.environ.get("XVLA_INSTRUCTION_EMBEDDING_RUNNER_SHA256")
    if expected_common != file_sha256(
        Path(__file__).with_name("instruction_embedding_ablation_v1_common.py")
    ):
...
def named_tensor_identity(model: Any) -> dict[str, str]:
    return {
        "learned_common_bos": tensor_sha256(model.bos),
        "token_embedding_weights": tensor_sha256(model.tok_emb.weight),
        "action_queries": tensor_sha256(model.action_queries),
        "joint_position_embeddings": tensor_sha256(model.pos_emb),
        "vision_position_embeddings": tensor_sha256(model.vision.pos_emb),
        "state_projection_weight": tensor_sha256(model.state_proj.weight),
        "state_projection_bias": tensor_sha256(model.state_proj.bias),
    }
...
class LexicalEmbeddingAblator:
    """Scoped post-lookup hook that zeros every instruction-embedding output."""

    def __init__(self, model: Any, expected_ids: list[int], torch: Any) -> None:
        self.model = model
        self.expected_ids = list(expected_ids)
        self.torch = torch
        self.handle = None
        self.records: list[dict[str, Any]] = []
        self.preexisting_hook_count = len(model.tok_emb._forward_hooks)
        if self.preexisting_hook_count != 0:
            raise RuntimeError("tok_emb already has a forward hook")

    def _hook(self, module: Any, inputs: tuple[Any, ...], output: Any) -> Any:
        if module is not self.model.tok_emb or len(inputs) != 1:
            raise RuntimeError("Lexical hook module or argument contract differs")
        ids = inputs[0]
        if ids.ndim != 2 or ids.shape[1] != PROTOCOL["instruction_token_count"]:
            raise RuntimeError("Lexical hook did not receive exactly 32 token positions")
        expected = self.torch.tensor(
            [self.expected_ids], dtype=self.torch.long, device=ids.device
        ).expand(ids.shape[0], -1)
        if not self.torch.equal(ids, expected):
            raise RuntimeError("Lexical hook received instruction IDs other than the correct prompt")
        if output.shape != (ids.shape[0], ids.shape[1], self.model.cfg.dim):
            raise RuntimeError("Lexical embedding output shape differs")
        replacement = self.torch.zeros_like(output)
        if int(self.torch.count_nonzero(replacement).item()) != 0:
            raise RuntimeError("The post-lookup replacement is not exactly zero")
        ids_array = ids.detach().cpu().contiguous().numpy()
        output_array = output.detach().cpu().contiguous().numpy()
        replacement_array = replacement.detach().cpu().contiguous().numpy()
        self.records.append(
            {
                "call_index": len(self.records),
                "input_ids_shape": list(ids.shape),
                "input_ids_sha256": hash_array(ids_array),
                "pre_ablation_output_shape": list(output.shape),
                "pre_ablation_output_dtype": str(output.dtype),
                "pre_ablation_output_sha256": hash_array(output_array),
                "pre_ablation_output_max_abs": float(output.detach().abs().max().item()),
                "returned_output_sha256": hash_array(replacement_array),
                "returned_output_max_abs": float(replacement.detach().abs().max().item()),
                "returned_nonzero_elements": int(
                    self.torch.count_nonzero(replacement).item()
                ),
                "zeroed_token_positions": int(ids.shape[0] * ids.shape[1]),
            }
        )
        return replacement

    def __enter__(self) -> "LexicalEmbeddingAblator":
        self.handle = self.model.tok_emb.register_forward_hook(self._hook)
        if len(self.model.tok_emb._forward_hooks) != self.preexisting_hook_count + 1:
            raise RuntimeError("Lexical hook installation failed")
        return self

    def __exit__(self, exc_type: Any, exc: Any, traceback: Any) -> None:
        if self.handle is not None:
            self.handle.remove()
        if len(self.model.tok_emb._forward_hooks) != self.preexisting_hook_count:
            raise RuntimeError("Lexical hook removal failed")

    def audit(self) -> dict[str, Any]:
        if not self.records:
            raise RuntimeError("The lexical ablation hook was never invoked")
        return {
            "enabled": True,
            "mechanism": "scoped_post_lookup_forward_hook_returning_torch_zeros_like",
            "hook_calls": len(self.records),
            "instruction_token_positions_zeroed_per_call": PROTOCOL[
                "instruction_token_count"
            ],
            "preexisting_hook_count": self.preexisting_hook_count,
            "post_context_hook_count": len(self.model.tok_emb._forward_hooks),
            "all_returned_outputs_exactly_zero": all(
                row["returned_nonzero_elements"] == 0
                and row["returned_output_max_abs"] == 0.0
...
def execute_condition(
    ...
    if condition not in CONDITIONS:
        raise ValueError("Unknown lexical-ablation condition")
    component_before = named_tensor_identity(model)
    if condition == "full_lexical_embeddings":
        hook_count = len(model.tok_emb._forward_hooks)
        if hook_count != 0:
            raise RuntimeError("Full-path condition unexpectedly has a tok_emb hook")
        value = callback()
        audit = {
            "enabled": False,
            "mechanism": "deployed_model_unmodified",
            "hook_calls": 0,
            "preexisting_hook_count": hook_count,
            "post_context_hook_count": len(model.tok_emb._forward_hooks),
            "all_returned_outputs_exactly_zero": None,
            "calls": [],
        }
    else:
        with LexicalEmbeddingAblator(model, instruction_ids, torch) as ablator:
            value = callback()
        audit = ablator.audit()
    component_after = named_tensor_identity(model)
    if component_before != component_after:
        raise RuntimeError("A model component changed during a condition")
    audit["protected_component_sha256_before"] = component_before
    audit["protected_component_sha256_after"] = component_after
    return value, audit
...
def main() -> None:
    ...
    encoded_prompts = {task: encode(text) for task, text in languages.items()}
    if any(len(ids) != PROTOCOL["instruction_token_count"] for ids in encoded_prompts.values()):
        raise RuntimeError("Correct prompt encoding does not contain exactly 32 positions")
    ...
    model.eval()
    if model.cfg.max_instr_len != PROTOCOL["instruction_token_count"]:
        raise RuntimeError("Model instruction sequence length differs")
    model_state_start = model_state_sha256(model)
    protected_start = named_tensor_identity(model)
    ...
    for task_index in range(args.task_start, args.task_end):
        ...
        try:
            for episode in range(args.episode_start, args.episode_start + args.eps_per_task):
                ...
                if (
                    input_sha != mapping["settled_physical_input_sha256"]
                    or components != mapping["settled_component_sha256"]
                    or hash_array(np.asarray(image, dtype=np.uint8))
                    != mapping["settled_model_image_sha256"]
                    or hash_array(np.asarray(raw_state, dtype=np.float32))
                    != mapping["settled_model_state_sha256"]
                ):
                ...
                instruction_ids = encoded_prompts[task_index]
                input_contract = {
                    "physical_input_sha256": input_sha,
                    "model_image_sha256": hash_array(np.asarray(image, dtype=np.uint8)),
                    "model_robot_state_sha256": hash_array(
                ...
                if args.mode == "strict_smoke":
                    ...
                    for condition in order:
                        ...
                        expected_hook_calls = int(condition == "zeroed_lexical_embeddings")
                        if audit["hook_calls"] != expected_hook_calls:
                            raise RuntimeError("Smoke hook-call count differs")
                ...
                conditions = {}
                for condition in order:
                    result, audit = execute_condition(
                        model,
                        condition,
                        instruction_ids,
                        lambda: rollout(
                    ...
                    if audit["hook_calls"] != expected_calls:
                        raise RuntimeError("Hook-call count differs from action-chunk count")
                    conditions[condition] = {
                        "prompt_id": task_index,
                        "prompt_text": languages[task_index],
                        "instruction_ids": instruction_ids,
    ...
    if args.mode == "strict_smoke":
        ...
        if len(rows) != 20 or smoke_rows:
            raise RuntimeError("Full shard does not contain exactly twenty paired rows")
        completed_rollouts = 2 * len(rows)
        if completed_rollouts != args.expected_rollouts:
            raise RuntimeError("Full rollout count differs")

    model_state_end = model_state_sha256(model)
    protected_end = named_tensor_identity(model)
    if model_state_start != model_state_end or protected_start != protected_end:
    ...
    output = {
        ...
        "identity": {
            ...
            "model_state_sha256_start": model_state_start,
            "model_state_sha256_end": model_state_end,
            "protected_component_sha256_start": protected_start,
            "protected_component_sha256_end": protected_end,
        },
        "evaluation": {
        ...
        "claim_boundary": (
            "This tests whether the learned lexical-embedding path is necessary for "
            "closed-loop success on familiar LIBERO-Object tasks, prompts, objects, "
            "scenes, and three fixed policies. It does not test semantic grounding, "
            "paraphrases, unseen objects, new compositions, cross-suite or physical "

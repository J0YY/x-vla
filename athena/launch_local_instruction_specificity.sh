...
for output in \
  results/local_instruction_specificity_smoke.json \
  results/local_instruction_specificity_s0.json \
  results/local_instruction_specificity_s1.json \
  results/local_instruction_specificity_s2.json \
  results/local_instruction_specificity_summary.json; do
  if [[ -e "$output" ]]; then
...
smoke_job=$(sbatch --parsable \
  --dependency="afterok:${provenance_job}" \
  --job-name=xvla-local-language-smoke \
  ...
  --output results/local_instruction_specificity_smoke.json \
  --checkpoint-seed 0 \
  --mode strict_smoke \
  --task-start 0 \
  --task-end 1 \
...
for seed in 0 1 2; do
  full_jobs[$seed]=$(sbatch --parsable \
    ...
    --job-name="xvla-local-language-s${seed}" \
    athena/slurm_local_instruction_specificity.sbatch \
    ...
    --provenance-result "$provenance_result" \
    --expected-provenance-job-id "$provenance_job" \
    --output "results/local_instruction_specificity_s${seed}.json" \
...
summary_job=$(sbatch --parsable \
  ...
  --job-name=xvla-local-language-summary \
  athena/slurm_summarize_local_instruction_specificity.sbatch \
  --result results/local_instruction_specificity_s0.json \
  --result results/local_instruction_specificity_s1.json \
  --result results/local_instruction_specificity_s2.json \
  --output results/local_instruction_specificity_summary.json)

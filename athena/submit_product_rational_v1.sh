...
readonly project_root=/work/joy/x-vla-product-pade-rational-v1
readonly run_root="$project_root/athena/results/product_pade_rational_v1"
...
"$python_executable" -B athena/prepare_product_rational_launch.py \
  ...
  --run-root athena/results/product_pade_rational_v1

mkdir "$run_root/gates/submission.lock"

smoke_train_raw=$(sbatch --parsable --time=01:00:00 \
